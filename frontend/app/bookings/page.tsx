...
export default function BookingsPage() {
    ...
    const fetchBookings = async () => {
        try {
            ...
            const [myRes, publishedRes] = await Promise.all([
                ...
                fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bookings/my-items`, { headers }),
            ]);
    ...
    const BookingCard = ({ booking, isOwner }: { booking: Booking; isOwner: boolean }) => (
            ...
            <div className="flex">
                ...
                <div className="flex-1 p-4">
                    <div className="flex items-start justify-between">
                        <div>
                            <Link href={`/items/${booking.itemId}`} className="text-lg font-bold text-gray-800 hover:text-blue-600 transition-colors">
                                {booking.itemName}
                            </Link>
                            <p className="text-sm text-gray-500 mt-1">
                                {isOwner ? (
                                    <>Requested by <span className="font-medium text-gray-700">{booking.userName}</span></>
                                ) : (
                                    <>Booking #{booking.id}</>
                                )}
                            </p>
                    ...
                    <div className="flex items-center gap-4 mt-3 text-sm text-gray-600">
                        ...
                        {booking.price && (
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="font-medium text-gray-800">€{booking.price.toFixed(2)}</span>
                            </div>
                        )}
                    </div>
    ...
    const currentBookings = activeTab === "my-bookings" ? myBookings : publishedBookings;
            ...
            <div className="min-h-screen">
                ...
                <div className="max-w-5xl mx-auto px-6 py-10 space-y-8">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 mt-2 text-lg">Manage your reservations and rental requests</p>
                    </div>
                    ...
                    <div className="flex gap-2 bg-white/60 backdrop-blur-xl rounded-2xl p-1.5 border border-white/50 shadow-lg w-fit">
                        ...
                        >
                            My Bookings
                            {myBookings.length > 0 && (
                                <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${activeTab === "my-bookings" ? "bg-white/20" : "bg-blue-100 text-blue-600"
                                    }`}>
                                    {myBookings.length}
                                </span>
                            )}
                        ...
                        >
                            ...
                            {publishedBookings.length > 0 && (
                                <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${activeTab === "my-items" ? "bg-white/20" : "bg-emerald-100 text-emerald-600"
                                    }`}>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-4">
                            {currentBookings.map((booking) => (
                                <BookingCard key={booking.id} booking={booking} isOwner={isOwnerView} />
                            ))}
                        </div>
                    )}
                </div>
