...
export default function Sidebar() {
  ...
  // Build nav items based on user role - admin sees only admin options
  ...
  return (
    <aside className="relative h-screen w-64 overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex flex-col h-full p-5">
        ...
        {/* Bottom Section */}
        <div className="mt-auto pt-4 border-t border-gray-200/50 dark:border-slate-700/50 space-y-2">
          {/* Settings Link */}
          <Link
            href="/profile"
            className={`
          ...
          >
            <span className={`
              transition-transform duration-300 group-hover:scale-110
              ${isActive("/profile") ? "text-white" : "text-gray-400 group-hover:text-blue-500 dark:group-hover:text-blue-400"}
            `}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50/80 dark:bg-slate-800/50 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors cursor-pointer group">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-md overflow-hidden">
              {profilePhoto ? (
                <img
                  src={profilePhoto}
                  alt="Profile"
