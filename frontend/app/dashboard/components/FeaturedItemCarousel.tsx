...
export default function FeaturedItemCarousel() {
            ...
            <div className="relative h-[480px] rounded-[2rem] overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900">
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <p className="text-xl font-semibold text-white/60">No items available</p>
                        <p className="text-sm text-white/40 mt-2">Check back later for new gear</p>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="relative group">
            ...
            <div className={`relative h-[480px] rounded-[2rem] overflow-hidden transition-all duration-700 ease-out ${isTransitioning ? 'opacity-0 scale-[0.98] blur-sm' : 'opacity-100 scale-100 blur-0'}`}>
                        ...
                        <div className="flex items-center gap-8">
                            {currentItem.price !== undefined && (
                                <div className="relative">
                                    ...
                                    <span className="absolute -top-1 -right-12 text-xs font-medium text-white/50 uppercase tracking-wider">/day</span>
                                </div>
                            )}
                            <Link
            ...
            <div className="flex justify-center items-center gap-3 mt-6">
                {items.slice(0, Math.min(items.length, 8)).map((_, idx) => (
                    <button
                        ...
                        className={`relative h-3 rounded-full transition-all duration-500 overflow-hidden ${idx === currentIndex
                            ? 'w-12'
                            : 'w-3 hover:w-4'
                            }`}
                    >
                        <div className={`absolute inset-0 transition-all duration-300 ${idx === currentIndex
                ...
                {items.length > 8 && (
                    <span className="text-xs text-gray-400 ml-2 font-medium">+{items.length - 8}</span>
                )}
            </div>
        </div>
    );
